"""
train date2vec-PE-E-SBERT

Note:
- dimension mismatch bewteen date2vec and BERT, so we only apply CM to Date2vec
"""
import sys
sys.path.insert(0,"../")
...
class PositionTimeESBert(nn.Module):
    ...
    def __init__(self, esbert_model, time_model, fuse_method="selfatt_pool", device="cuda", freeze_time_module=False):
        ...
        if "selfatt_pool" in fuse_method:
    ...
    def forward(self, features):
        ...
        # if self.fuse_method == "additive_selfatt_pool":
        ...
        if self.fuse_method == "selfatt_pool":
            repeat_time_vec = time_features.unsqueeze(1).repeat(1, token_embeddings.shape[1], 1)
...
def main():
    ...
    elif args.dataset_name == "tdt4": # default TDT4
...
#         test_corpus = pickle.load(handle)
    ...
    time_position_model = Date2VecConvert(model_path="./Date2Vec/d2v_model/d2v_98291_17.169918439404636.pth")
    ...
    exp_model = "exp_date2vec_fronzen_esbert" if args.freeze_time_module else "exp_date2vec_esbert"
